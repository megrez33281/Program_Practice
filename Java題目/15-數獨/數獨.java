...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       String sudo_str = read("sudoI");//數獨的檔案名稱
    ...
    private static void find_answer(char[] sudo_one_arr){
        ...
        if(sudo_str_read.length() < 80) {
            ...
            if (emptyI != 0) {
                ...
                int least_locate = find_the_least_locate(sudo_one_arr,leastI);
                char[] possible_number = judge(sudo_one_arr,least_locate);
                char[][] possible_answer = new char[leastI][81];
                for (int i = 0; i < leastI; i++) {
                    System.arraycopy(sudo_one_arr, 0, possible_answer[i], 0, 81);
                }
                for(int i=0; i<leastI; i++){
                    possible_answer[i][least_locate] = possible_number[i]; }
